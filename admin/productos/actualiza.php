...
if ($stm->execute([$nombre, $descripcion, $precio, $descuento, $stock, $categoria, $id])) {
    ...
    if (
        $sizeTalla == count($color) && $sizeTalla == count($precioVariante)
        && $sizeTalla == count($stockVariante)
    ) {
        $sql = "INSERT INTO productos_variantes (id_producto, id_talla, id_color, precio, stock)
        VALUES (?,?,?,?,?)";
        $stm = $con->prepare($sql);

        $sqlUpdate = "UPDATE productos_variantes SET id_talla=?, id_color=?, precio=?, stock=?
        WHERE id = ?";
